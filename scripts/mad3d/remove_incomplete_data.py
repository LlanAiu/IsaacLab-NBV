...
def remove_objaverse_invalid_folders(root_dir):
    ...
    for top_folder in tqdm(os.listdir(root_dir)):
        ...
        for instance_folder in tqdm(os.listdir(top_folder_path), leave=False):
            ...
            if not os.path.exists(usd_file):
                tqdm.write(f"Removing {instance_folder_path} due to missing .usd file")
                count += 1
                shutil.rmtree(instance_folder_path, ignore_errors=True)
                continue

            # Condition 2: Check if the folder has the occ.npy file
            if not os.path.exists(occ_file):
                tqdm.write(f"Removing {instance_folder_path} due to missing occ.npy")
                count += 1
                shutil.rmtree(instance_folder_path, ignore_errors=True)
                continue
            ...
            if np.sum(occ_grid[:, :, 1:]) == 0:
                tqdm.write(f"Removing {instance_folder_path} due to empty occupancy grid")
                count += 1
                shutil.rmtree(instance_folder_path, ignore_errors=True)
                continue
            ...
            if geometry_prim:
                ...
                if invalid_nodes:
                    tqdm.write(f"Removing {instance_folder_path} due to invalid nodes under geometry: {invalid_nodes}")
                    count += 1
                    shutil.rmtree(instance_folder_path, ignore_errors=True)
                    continue            
    print(count)
